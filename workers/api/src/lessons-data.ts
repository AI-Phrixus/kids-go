...
export const LESSONS: LessonMeta[] = [
  ...
  },
  {
    ...
    skillTag: T.title("官子與數目（畢業）", "ヨセと目算（卒業）", "Endgame & counting (graduation)"),
    titles: T.title("長安凱旋 · 數寶大典", "長安凱旋 · 地を数える", "Triumphant return · Counting territory"),
    story: T.story(
      "{{name}}，回到長安了！最後一課用**日本規則**數目：**地（圍住的空點）＋提子（アゲハマ）**。邊界只要有一個缺口，整片地就會漏光——把缺口補起來，才算你的！",
      "{{name}}、長安に帰ってきた！最後は日本ルールの目算：**地（囲った空点）＋アゲハマ**。境界に穴が一つでもあると地は全部流れる。穴をふさいで自分の地にしよう！",
      "{{name}}, back in Chang'an! The final lesson uses Japanese rules: **territory (surrounded empty points) + prisoners**. One gap in your border leaks the whole area — seal every gap to keep it!",
    ),
    goal: T.story("補好邊界所有缺口，靠地數領先獲勝", "境界の穴を全部ふさぎ、地で勝つ", "Seal every border gap and win on territory"),
    battle: {
      // v0.8.0 (Japanese rules): black's wall has TWO leaks; each unsealed gap
      // leaks the ENTIRE left area into neutral. Black must seal both to lock
      // its 27 目; white seals its own gap for 18. Black wins by 9. Fully
      // scripted (deterministic) so the graduation count is clean.
      mode: "sequence",
      aiLevel: 2,
      par: 3,
      afterScript: "won",
      ...
      script: [
        {
          expect: [{ x: 3, y: 4 }],
          reply: { x: 6, y: 4 },
          hint: T.story("你的左牆在 (3,4) 有個洞——先補這裡！", "左の壁の穴(3,4)をふさごう！", "Your left wall has a hole at (3,4) — seal it first!"),
        },
        {
          expect: [{ x: 3, y: 2 }],
          hint: T.story("還有一個洞在 (3,2)！補上，左邊 27 目才真的是你的。", "もう一つの穴(3,2)！ふさげば左の27目が確定。", "One more hole at (3,2)! Seal it and the left 27 points are truly yours."),
          sayKey: "graduate",
        },
      ],
    },
    steps: [
      { type: "story" },
      info(
        "日本規則數目：你的**地**（被你圍住、只碰到你棋子的空點）＋你的**提子**（吃掉對方的子）。棋子本身不算目。",
        "日本ルール：**地**（自分だけが囲った空点）＋**アゲハマ**（取った石）。石そのものは数えない。",
        "Japanese rules: your **territory** (empty points bordered only by you) + your **prisoners** (stones you captured). Stones themselves are not counted.",
      ),
      info(
        "官子＝把邊界關好。邊界只要漏一個洞，整片空就會「碰到對方」變成單官（不算誰的）！",
        "ヨセ＝境界を閉じる。穴が一つでもあると、その空は相手に触れてダメ（誰の地でもない）になる！",
        "Endgame = seal borders. A single gap lets the whole area touch the opponent and become dame (nobody's)!",
      ),
      info(
        "補完所有洞 → 沒棋可下就 pass → 兩人都 pass → 一起數地！",
        "全部ふさぐ → 打つ所がなければパス → 二人パス → 地を数える！",
        "Seal all gaps → pass when nothing's left → two passes → count territory!",
      ),
    ],
  }
];
