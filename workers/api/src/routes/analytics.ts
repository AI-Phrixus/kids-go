...
analytics.post("/events", async (c) => {
  ...
  const body = await c.req.json<{ event?: string; payload?: unknown }>().catch(() => ({}));
  const event = String((body as { event?: string }).event || "");
  if (!ALLOWED.has(event)) return c.json({ error: "invalid_event" }, 400);
  const id = uid();
  const payload = JSON.stringify((body as { payload?: unknown }).payload ?? {});
...
analytics.get("/stats", async (c) => {
  ...
  for (const r of rows.results ?? []) map[r.event_type] = r.n;
  ...
  return c.json({
    ...
    summary: {
      ...
      /** rough engagement signal for parents */
      breakPerLesson:
